/**
 * ChartPanel — Single Responsibility: render OHLCV chart + EMA lines + signal markers.
 * Bridges LightweightCharts (imperative) with React (declarative) using refs.
 */
...
export default function ChartPanel({ symbol, interval, strategy }) {
  ...
  const { data, loading, error } = useChartData(symbol.yahoo, interval, strategy);
  ...
  // ── Load chart data when it changes ───────────────────────────────────────
  useEffect(() => {
  ...
  // ── Live candle update from WS tick ───────────────────────────────────────
  useEffect(() => {
  ...
  return (
    <div className="chart-wrap">
      <div className="chart-toolbar" title="All times shown in local timezone">
        ...
        {sigCount === 0 && !loading && <span className="ct-sigs" style={{ color: "var(--dim)" }}>No signals on this TF</span>}
      </div>
