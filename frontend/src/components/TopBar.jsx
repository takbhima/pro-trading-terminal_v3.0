/**
 * TopBar — Single Responsibility: symbol search, TF selection, connection status.
 */
...
export default function TopBar({ symbol, interval, onScan, onIntervalChange }) {
  ...
  // Keep input in sync when wl click changes symbol
  useEffect(() => { setInput(symbol.yahoo); }, [symbol.yahoo]);

  // Re-subscribe when symbol or interval changes
  useEffect(() => {
    subscribe(symbol.yahoo, curTF);
  }, [symbol.yahoo, curTF, subscribe]);
  ...
  // Live tick display — only for current symbol
