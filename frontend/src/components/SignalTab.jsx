/**
 * SignalTab — FIX: Live WS signal pushes are now filtered by strategy.
 * A signal from the backend that was generated by a different strategy
 * (e.g. leftover pro_mtf signal after user switched to macd) is ignored
 * so the "Latest Signal" panel doesn't show stale cross-strategy data.
 *
 * FIX: SignalHistoryCard now passes the symbol (yahoo ticker) when calling
 * onJumpToSignal so App.jsx can switch the active chart symbol if needed.
 */
...
// ── Signal History Card ──────────────────────────────────────────────────────
function SignalHistoryCard({ sig, label, activeTrade, onJumpToSignal }) {
  const isBuy = sig.type === "BUY" || sig.signal_type === "BUY";

  // Determine trade status:
  // 1. Use backend-supplied trade_status if available
  // 2. Cross-check with activeTrade: if entry_price matches this signal, it's open
  // 3. Otherwise closed
  const isOpen = sig.trade_status === "open" || (
    activeTrade &&
    Math.abs(parseFloat(activeTrade.entry_price) - parseFloat(sig.price)) < 0.01
  );

  const handleClick = () => {
    if (onJumpToSignal && sig.time) {
      // FIX: pass the signal's symbol (yahoo ticker) so App.jsx can switch
      // the active chart to the correct stock before scrolling to the signal.
      // sig.symbol may be a display label; sig.yahoo_symbol is the ticker if set,
      // otherwise fall back to sig.symbol (which is the yahoo ticker in history cards).
      const symbolYahoo = sig.yahoo_symbol || sig.symbol || label;
      const symbolLabel = sig.display_name || sig.symbol || label;
      onJumpToSignal(sig.time, sig.signal_id, symbolYahoo, symbolLabel);
    }
  };
  ...
  return (
    <div
      ...
      onClick={handleClick}
      title={onJumpToSignal ? "Click to load this stock's chart and jump to signal" : undefined}
    >
      {/* Open / Closed status badge */}
      <div className="sig-status-row">
        <span className={`sig-status-badge ${isOpen ? "open" : "closed"}`}>
          {isOpen ? "● OPEN" : "✓ CLOSED"}
        </span>
        {onJumpToSignal && (
          <span className="sig-jump-hint">↗ Jump to chart</span>
        )}
      </div>

      <div className="sig-row">
        <b>{sig.display_name || sig.symbol || label}</b>
      ...
      {sig.target_time && (
        <div className="sig-row">
          <span>⏱ Target</span>
          <span className="v" style={{ color: "var(--yellow)" }}>{sig.target_time}</span>
        </div>
      )}
...
// ── Main SignalTab ───────────────────────────────────────────────────────────
export default function SignalTab({ symbol, interval, strategy, requireMtf, fetchKey = 0, onJumpToSignal }) {
  ...
  // Clear signal display on ANY dimension change
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (responseStrategy && responseStrategy !== strategy) return;
    if (responseInterval  && responseInterval  !== interval) return;
    ...
    if (data.signals?.length) {
      const historical = [...data.signals]
        ...
        .map((s) => ({
          ...s,
          // Store both yahoo ticker and display label for cross-symbol navigation
          yahoo_symbol: symbol.yahoo,
          display_name: symbol.label,
          symbol: symbol.yahoo,  // keep yahoo ticker as symbol for jump handler
    ...
    if (data.latest_signal) {
      setHeroSignal({ ...data.latest_signal, yahoo_symbol: symbol.yahoo, display_name: symbol.label, symbol: symbol.yahoo });
      setSigStrategy(data.latest_signal.strategy || strategy);
    }
  }, [data, symbol.yahoo, symbol.label, interval, strategy, requireMtf, fetchKey]);

  // Live WS signal handling
  useEffect(() => {
    if (!lastSignal) return;
    const sigStrategyKey = lastSignal.strategy || "pro_mtf";
    // Discard signals from a different strategy than what user has selected
    if (sigStrategyKey !== strategy) return;

    // CRITICAL FIX: The backend sends the yahoo ticker as lastSignal.symbol
    // (e.g. "*BSESN" for SENSEX, "BTC-USD" for Bitcoin).
    // We must use this to determine which stock this signal belongs to.
    // If lastSignal.symbol is missing, this is a signal for the current symbol.
    const sigYahoo = lastSignal.symbol || symbol.yahoo;

    // CRITICAL FIX: Only add to history and update hero if this signal
    // belongs to the CURRENTLY SELECTED symbol. Signals for other watchlist
    // symbols must be ignored here — they will be fetched when user clicks
    // that stock in the watchlist.
    if (sigYahoo !== symbol.yahoo) return;
    ...
    const enriched = {
      ...
      trade_status: "open",
      yahoo_symbol: symbol.yahoo,       // always use current symbol (already verified match above)
      display_name: symbol.label,       // use the proper display name, not the raw ticker
      symbol:       symbol.yahoo,
    };
    setHeroSignal(enriched);
    setSigHistory((prev) => [enriched, ...prev].slice(0, 60));
    if (Notification.permission === "granted") {
      new Notification(`${enriched.type} — ${symbol.label}`, {
        body: `Entry: ${fmt(enriched.price)}  SL: ${fmt(enriched.sl)}  TP: ${fmt(enriched.tp)}`,
      });
    }
  // CRITICAL FIX: symbol.yahoo and symbol.label MUST be in dep array.
  // Without them the closure captures the old symbol and tags every signal
  // with the wrong stock (e.g. SENSEX signals appear under Bitcoin).
  }, [lastSignal, strategy, symbol.yahoo, symbol.label]);

  // When an active trade closes, mark its signal card as closed
  useEffect(() => {
    if (!lastExitEv) return;
    setSigHistory((prev) => prev.map((s) =>
      Math.abs(parseFloat(s.price) - parseFloat(lastExitEv.entry_price)) < 0.01
