...
function SignalHistoryCard({ sig, label, activeTrade, onJumpToSignal }) {
  ...
  const handleClick = () => {
    if (onJumpToSignal && sig.time) {
      onJumpToSignal(sig.time, sig.signal_id);
    }
  };
...
export default function SignalTab({ symbol, interval, strategy, requireMtf, fetchKey = 0, onJumpToSignal }) {
  ...
  // FIX: Live WS signal — only accept if it matches the currently selected strategy
  useEffect(() => {
    if (!lastSignal) return;
    const sigStrategy = lastSignal.strategy || "pro_mtf";
    // FIX: discard if this signal is from a different strategy than what user has selected
    ...
    const enriched = {
      ...
      trade_status: "open",  // live signals are always fresh/open
    };
  ...
  }, [lastSignal, strategy]);  // FIX: strategy in dep array so stale closure can't sneak through
